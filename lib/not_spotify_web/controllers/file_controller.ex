defmodule NotSpotifyWeb.FileController do
  @moduledoc """
  ...
  def show(conn, %{"id" => filename_uuid, "token" => token}) do
    path = Media.local_filepath(filename_uuid)
  ...
  defp proxy_file(conn, ip, mime_type, content_length) do
    ...
    port = NotSpotifyWeb.Endpoint.config(:http)[:port]
  ...
  defp server_ip, do: NotSpotify.config([:files, :server_ip])

  defp file_server_opts do
    [
      hostname: NotSpotify.config([:files, :hostname]) || "localhost",
      transport_opts: NotSpotify.config([:files, :transport_opts]) || []
    ]
  end
end
