defmodule NotSpotifyWeb.SongLive.UploadFormComponent do
  ...
  def handle_event("validate", %{"_target" => ["mp3"]}, socket) do
    ...
    new_socket =
      ...
      |> Enum.reduce(socket, fn entry, acc ->
        put_new_changeset(acc, entry)
      end)
      |> drop_invalid_uploads()
  ...
  defp put_new_changeset(socket, entry) do
    cond do
      ...
      true ->
        attrs = Media.parse_file_name(entry.client_name)
        changeset = Media.change_song(%Song{}, attrs)
  ...
  defp async_calculate_duration(socket, %Phoenix.LiveView.UploadEntry{} = entry) do
    ...
    consume_uploaded_entry(socket, entry, fn %{path: path} ->
      Task.Supervisor.start_child(NotSpotify.TaskSupervisor, fn ->
  ...
  defp put_stats(socket, entry_ref, %MP3Stat{} = stat) do
    if changeset = get_changeset(socket, entry_ref) do
      case Media.put_stats(changeset, stat) do
        {:ok, new_changeset} ->
  ...
  defp drop_invalid_uploads(socket) do
    ...
    Enum.reduce(uploads.mp3.entries, socket, fn entry, socket ->
